...
- [x] 1. 현재 로그 발생 지점과 마이그레이션 범위를 코드 기준으로 동결
...
- [x] 2. Canonical event schema와 event taxonomy를 실코드 기준 상수/헬퍼로 정의
...
- [x] 3. 중앙 로깅 설정을 `dictConfig()` 기반으로 교체하되 기존 파일 확장을 우선
...
- [x] 4. run/phase/provider/attempt 컨텍스트를 `contextvars`로 자동 주입
...
- [x] 5. Queue 기반 sink topology를 실제 코드에 맞게 설계하고 적용
...
- [x] 6. Console renderer와 JSONL renderer를 같은 event dict 위에 구현
...
- [x] 7. 민감정보 redaction과 payload truncation을 공통 filter/helper로 구현
...
- [x] 8. `PipelineObserver`를 summary/audit 중심으로 재정의하고 stdout 직접 출력 제거 순서를 고정
...
- [x] 9. GitHub Actions wrapper와 app 로그를 `Run pipeline` step에서 논리적으로 분리
...
- [x] 10. 최종 stdout summary 계약을 단일 event로 명시하고 중복 없이 한 번만 출력
...
- [x] 11. 모든 `logger` 레벨 호출을 canonical logging layer 위에 올리도록 모듈별로 마이그레이션
...
- [x] 12. JSONL artifact와 observability summary를 연결하고 source of truth 전환 단계를 문서화
...
- [x] 13. 테스트 작성
  ...
  - [x] 13.1 logging setup / config 단위 테스트
  ...
  - [x] 13.4 redaction / truncation 테스트
    - 시크릿 값이 마스킹되는지 확인
  ...
  - [x] 13.10 프로젝트 전체 로그 surface 스캔 테스트
