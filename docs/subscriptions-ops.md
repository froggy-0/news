...
3. Cloudflare Pages 프로젝트에 `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `CONFIRMATION_SES_SENDER`를 secret로 등록합니다.
...
3. workflow 실행 후 SES 발송과 실제 수신 mailbox 도착 여부를 확인합니다.

### Cloudflare preview confirmation 발송

1. preview 환경에 `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `CONFIRMATION_SES_SENDER`가 모두 등록됐는지 확인합니다.
