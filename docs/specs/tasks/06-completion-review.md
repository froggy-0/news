...
> 백필 + sentiment-join 재실행으로 docs/reports/sentiment-join-code-report-draft.md가 완성되는지 검증합니다.
...
## 리포트 빈칸 해소 여부 (docs/reports/sentiment-join-code-report-draft.md 섹션별)
...
tasks 수정 → 백필 → sentiment-join 후, `docs/reports/sentiment-join-code-report-draft.md`의 빈칸(TBD, —)을 실제 값으로 채우는 자동화가 없습니다. 수동으로 `extract_report_data.py` 출력을 복사해야 합니다.
...
docs/reports/sentiment-join-code-report-draft.md §4.2에서 "직접 인과성 검정이 필요하면 GRANGER_PAIRS에 추가 필요"라고 명시했지만, tasks 문서 어디에도 이 수정이 포함되어 있지 않습니다. 가이드라인의 "뉴스 감성 지표와 F&G Index 간의 Granger 인과성"을 직접 검정하려면 추가해야 합니다.
