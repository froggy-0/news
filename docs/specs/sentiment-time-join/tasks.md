...
- [x] 1. 모듈 디렉토리 및 의존성 세팅
  - [x] 1.1 디렉토리 구조 생성
    ...
    - _Requirements: 1.3_
  - [x] 1.2 `requirements-analysis.txt` 신규 파일 생성 및 CI 등록
    - `pandera>=0.18`
    ...
    - _Requirements: 8.1_
  - [x] 1.3 `tests/analysis/conftest.py` 생성
    ...
    - _Requirements: 1.1_
  - [x] 1.4 `Makefile`에 `sentiment-join` 타겟 추가
    ```makefile
    sentiment-join:
...
- [x] 2. `analysis/sentiment_join/config.py` 구현
  - [x] 2.1 `SentimentJoinSettings` dataclass 구현
  ...
  - [x] 2.2 `load_sentiment_join_settings()` 구현
    - `lookback_days` 범위 검증: 30 미만 또는 730 초과 시 `ValueError`
    - `dotenv` 로드 (기존 `config.py` 동일 방식)
    - _Requirements: 10.2_
  - [x] 2.3 `tests/analysis/test_sentiment_join/test_config.py` 작성
...
- [x] 4. `sources/r2_sentiment.py` 구현
  - [x] 4.1 `fetch_r2_sentiment(dates, r2_public_bucket, max_concurrency=10)` 구현
  ...
  - [x] 4.2 `tests/analysis/test_sentiment_join/test_r2_sentiment.py` 작성
...
- [x] 5. `sources/fng.py` 구현
  - [x] 5.1 `fetch_fng(lookback_days)` 구현
  ...
  - [x] 5.2 `tests/analysis/test_sentiment_join/test_fng.py` 작성
...
- [x] 6. `sources/btc_prices.py` 구현
  - [x] 6.1 `_fetch_coingecko_range(unix_start, unix_end)` 구현 (내부 함수)
    ...
    - _Requirements: 4.1_
  - [x] 6.2 `fetch_btc_close(start_date, end_date)` 구현 (공개 함수)
  ...
  - [x] 6.3 `tests/analysis/test_sentiment_join/test_btc_prices.py` 작성
...
- [x] 7. `sources/usdkrw_prices.py` 구현
  - [x] 7.1 `fetch_usdkrw_close(start_date, end_date, kis_app_key, kis_app_secret)` 구현
  ...
  - [x] 7.2 `tests/analysis/test_sentiment_join/test_usdkrw_prices.py` 작성
...
- [x] 9. `transform.py` 구현
  - [x] 9.1 `normalize_dates(df, date_col="date")` 구현
    - 타임존 포함 → UTC 변환 후 `YYYY-MM-DD` string 통일
    - 이미 string인 경우 포맷만 검증
    - _Requirements: 6.1_
  - [x] 9.2 `forward_fill_prices(df, cols, max_periods=2)` 구현
    ...
    - _Requirements: 6.2_
  - [x] 9.3 `compute_returns(df, price_col)` 구현
  ...
  - [x] 9.4 `trim_to_date_range(df, start_date, end_date)` 구현
    - `btc_start = start_date - 1일`로 수집한 extra row를 수익률 계산 후 제거
    - `df[df["date"] >= start_date]` 필터링
    - _Requirements: 4.1, 5.1_
  - [x] 9.5 `tests/analysis/test_sentiment_join/test_transform.py` 작성
...
- [x] 11. `join.py` 구현
  - [x] 11.1 `_compute_sources_used(dfs: dict[str, pd.DataFrame])` 구현 (내부 유틸)
  ...
  - [x] 11.2 `detect_outliers_rolling_iqr(df, cols, window=30, iqr_multiplier=3.0, min_periods=15)` 구현
    ...
    - _Requirements: 6.3_
  - [x] 11.3 `merge_sources(sentiment_df, fng_df, btc_df, usdkrw_df)` 구현
  ...
  - [x] 11.4 `tests/analysis/test_sentiment_join/test_join.py` 작성
...
- [x] 13. `validate.py` 구현
  - [x] 13.1 `MASTER_SCHEMA` pandera DataFrameSchema 정의
    ...
    - _Requirements: 8.1_
  - [x] 13.2 `validate_master(df)` 구현
    - `MASTER_SCHEMA.validate(df)` 호출
    - `SchemaError` → `ERROR` 로그 출력 후 re-raise (파이프라인이 종료 코드 1로 처리)
    - _Requirements: 8.2_
  - [x] 13.3 `tests/analysis/test_sentiment_join/test_validate.py` 작성
    - 정상 DataFrame → 검증 통과
...
- [x] 14. `storage.py` 구현
  - [x] 14.1 `save_parquet(df, output_dir, run_date, *, ffill_days=0)` 구현
  ...
  - [x] 14.2 `cleanup_old_files(output_dir, retain_days)` 구현
    - `retain_days=0` → 무동작
    - `output_dir.glob("master_*.parquet")` 순회 → 파일명에서 날짜 파싱 (`master_YYYYMMDD.parquet`)
    - 오늘 기준 `retain_days` 초과 파일 삭제
    - _Requirements: 9.2_
  - [x] 14.3 `upload_to_r2(local_path, r2_key, *, r2_s3_endpoint, r2_access_key_id, r2_secret_access_key, r2_public_bucket)` stub 구현
    - 함수 본체: `return` 한 줄 (no-op)
    - 시그니처에 접속 정보 파라미터 포함 — 향후 확장 시 시그니처 변경 불필요
    - _Requirements: 14.1_
  - [x] 14.4 `tests/analysis/test_sentiment_join/test_storage.py` 작성
...
- [x] 16. `pipeline.py` 구현
  - [x] 16.1 `run_sentiment_join(settings)` 구현
    - **단계 순서:**
      1. 날짜 범위 계산
         - `end_date = today`
  ...
  - [x] 16.2 `tests/analysis/test_sentiment_join/test_pipeline.py` 작성
...
- [x] 17. `scripts/build_sentiment_join.py` 구현
  - [x] 17.1 CLI 진입점 작성
    ```python
    #!/usr/bin/env python3
    import sys
    ...
    ```
    - _Requirements: 1.3_
  - [x] 17.2 실행 가능 권한 설정: `chmod +x scripts/build_sentiment_join.py`
  - [x] 17.3 dry-run 확인: 환경변수 미설정 상태에서 `ValueError` 없이 설정 로드 후 소스 수집 시작 여부만 확인

---

- [x] 18. Checkpoint 6 — 파이프라인 통합
...
- [x] 19. `make check` 전체 통과
  - [x] 19.1 `make fmt` — Ruff 자동 포매팅
  - [x] 19.2 `make lint` — Ruff 린트 검사
  - [x] 19.3 `make test` — 전체 pytest (기존 테스트 회귀 없음 확인)
  - [x] 19.4 `make typecheck` — mypy strict (신규 모듈 타입 오류 없음)
  - _Requirements: 전체_

- [x] 20. 기존 파이프라인 회귀 없음 확인
  - [x] 20.1 `pytest tests/ -v --ignore=tests/analysis` — 기존 테스트 전체 통과
  - [x] 20.2 `pipeline.py`, `main.py`, `config.py`, `public_site.py` diff 없음 확인
    ```bash
...
- [x] 22. Checkpoint 7 — 최종
  - `make check` 통과
  - 기존 테스트 회귀 없음
  - `docs/specs/sentiment-time-join/tasks.md` Overview 완료 일시 기록
