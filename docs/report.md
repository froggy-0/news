...
**이상치 정책 4종** (`SENTIMENT_JOIN_OUTLIER_POLICY` 환경변수로 선택):
...
**라벨 구분 임계값**:
- bullish: `score ≥ finbert_bullish_threshold` (settings에서 설정, 통상 +0.05)
- bearish: `score ≤ finbert_bearish_threshold` (통상 -0.05)
- neutral: 그 사이
...
**표준화**: 기본값은 `StandardScaler` (z-score, 평균 0·분산 1). 실험 플랫폼에서 `SENTIMENT_JOIN_SCALER_KIND=robust`로 전환하면 `RobustScaler` (median·IQR 기반)를 사용합니다.
