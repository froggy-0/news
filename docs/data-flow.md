...
│    ├─ 한국 투자자 참고 지표 (yfinance)                               │
│    ├─ 미국 지수 (Stooq → yfinance fallback)                         │
│    ├─ 빅테크 10종 (Stooq → yfinance fallback)                       │
│    └─ 비트코인 스냅샷                                                │
│        ├─ BTC 현물 (CoinGecko → yfinance fallback)                   │
│        ├─ BTC ETF 가격·거래량 (Stooq → yfinance)                    │
...
│  Phase 7: 이메일 발송                                               │
│  GmailSender.send()                                                 │
│    Markdown → HTML 렌더링 + plain text fallback → Gmail API 발송     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| `dxy` | 달러 인덱스 (ICE DXY) | — | `DX=F` | 포인트 |
| `vix` | 변동성 지수 | `VIXCLS` | `^VIX` | 포인트 |

FRED 성공 시 이미 가져온 키는 건너뛰고, 나머지(`us3m`, `dxy` 등)만 yfinance로 보충합니다.
...
### 1.2 한국 투자자 참고 지표 (`packet["korea_watch"]`)

| canonical_key | 지표 | 소스 | 비고 |
|---|---|---|---|
| `usdkrw` | 원/달러 환율 | yfinance `KRW=X` | |
| `nq_futures` | 나스닥 선물 | yfinance `NQ=F` | |

### 1.3 미국 지수 (`packet["us_indices"]`)

| canonical_key | 지표 | 소스 |
|---|---|---|
| `spy` | S&P 500 ETF | Stooq `spy.us` → yfinance `SPY` |
| `qqq` | Nasdaq 100 ETF | Stooq `qqq.us` → yfinance `QQQ` |
| `soxx` | 반도체 ETF | Stooq `soxx.us` → yfinance `SOXX` |

### 1.4 빅테크 10종 (`packet["tech_stocks"]`)

`NVDA`, `MSFT`, `AAPL`, `AMZN`, `GOOGL`, `META`, `AMD`, `TSM`, `ASML`, `AVGO`

- 소스: Stooq → yfinance fallback
...
```python
{
    ...
    "etf_points": [MarketPoint],  # IBIT, FBTC, ARKB, BITB, GBTC 가격
    "etf_total_volume": int | None,  # ETF 합산 거래량
    "fear_greed_value": int | None,  # 0~100
    "fear_greed_label": str | None,  # "Extreme Fear" ~ "Extreme Greed"
    # 공식 ETF 보유량 (IBIT+BITB direct fetch — Perplexity structured query 제거됨)
    "official_etf_snapshots": [BitcoinEtfIssuerSnapshot],
    "official_etf_total_btc": float | None,
    "official_etf_total_aum_usd": float | None,
    "official_etf_daily_flow_btc": float | None,   # 캐시 대비 일일 변동
    "official_etf_daily_flow_usd": float | None,
    "official_etf_supported_tickers": [str],
    "official_etf_compared_tickers": [str],
}
```
...
```python
{
    "ticker": str,              # IBIT / BITB (2종 운영, GBTC 제거됨)
    "issuer": str,              # BlackRock / Bitwise
...
| 데이터 | 갱신 주기 | 특성 | 활용 가능성 |
|---|---|---|---|
| 거시 지표 5종 | 실시간~1일 | 정량, 변동률 포함 | 추세 분석, 알림 조건 |
| 미국 지수 3종 | 실시간 | 정량, ETF 기반 | 섹터 비교 |
| 빅테크 10종 | 실시간 | 변동률 정렬됨 | 모멘텀 스크리닝 |
| BTC 현물 + ETF 5종 | 실시간 | 가격 + 거래량 | 유동성 분석 |
