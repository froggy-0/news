...
단일 LogisticRegression(C=0.3, class_weight="balanced")으로 피처 세트별 예측력을 검증한다. (research-only — `decision` 항상 `"research_only"`)

> **코드 현실**: 문서 초안에 언급된 L1Logistic + ElasticNet + LightGBM 앙상블은 `models.py`에 클래스만 구현됐으며 이 경로에서 호출되지 않는다. 표본(539행) 부족으로 앙상블 연결은 보류 상태.
