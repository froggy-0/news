...
| 최신 live version | code 기준 `strategy_version=arena-ec2-v6`, `params_version=arena-params-v7`, `feature_set_version=arena-features-v4`, `risk_model_version=portfolio-risk-v1` |
...
- EC2 재배포 완료.
- 과거 확인 run `89c6ca1d-a62b-4c1e-97ae-c5b3d5a563cf`는 `params_version=arena-params-v2`, `risk_model_version=portfolio-risk-v1`이었다. 현재 코드 기준은 상단 표의 `arena-params-v7`이므로 다음 live run에서 v7 기록을 확인한다.
...
- `BTCUSDT 1h 180d` dry-run 수집: 4,319 complete bars 파싱 완료.
- `research_1h` walk-forward: DB에 1H raw가 아직 없어 `insufficient_data`.
- `live_4h` backtest: `arena-ec2-v6`, `arena-params-v7`, `arena-features-v4`로 정상 완료.
...
| market-structure/frequency SQL 적용 필요 | vNext와 frequency 테이블은 새 migration 필요 | 사용자가 SQL Editor에서 실행 후 readiness 확인 |
...
1. **Step 5A: SQL 적용 상태 고정**
   - `/Users/giwon/code/news/supabase/migrations/20260620_arena_market_structure_v1.sql` 실행.
   - `/Users/giwon/code/news/supabase/migrations/20260620_arena_frequency_research_v1.sql` 실행.
   - `arena_market_structure_v1_ready`, `arena_shadow_vnext_ready` 확인.
   - `arena_frequency_research_v1_ready` 확인.
...
4. **Step 5D: frequency raw 수집과 research_1h 검증**
   - `BTCUSDT 1h 180d` raw OHLCV 저장.
   - `BTCUSDT 15m 90~180d` raw OHLCV 저장.
