...
| 최신 live version | code 기준 `strategy_version=arena-ec2-v7`, `params_version=arena-params-v9`, `feature_set_version=arena-features-v5`, `risk_model_version=portfolio-risk-v1` |
...
- `live_4h` backtest: `arena-ec2-v7`, `arena-params-v9`, `arena-features-v5` 기준 정상 완료.
...
- `ENABLE_ARENA_REALTIME_COLLECTOR=false`
...
| realtime execution SQL 적용 필요 | realtime feature/gate/order 원장은 새 migration 필요 | 사용자가 SQL Editor에서 실행 후 readiness 확인 |
...
1. **Step 5A: SQL 적용 상태 고정**
   - `/Users/giwon/code/news/supabase/migrations/20260620_arena_realtime_execution_v1.sql` 실행.
   - `arena_realtime_execution_v1_ready`, `arena_execution_gate_shadow_ready` 확인.
...
4. **Step 5D: realtime execution shadow 검증**
   - `arena_execution_gates` 최신 row 확인.
   - SQL 적용 후 `ENABLE_ARENA_REALTIME_COLLECTOR=true` 승격 여부 결정.
