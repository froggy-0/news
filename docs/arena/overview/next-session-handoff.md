...
8. `docs/arena/overview/decision-log.md`

## 현재 한 줄 상태

Arena는 EC2 상시 프로세스에서 `BTCUSDT` 4H paper trading을 돌고 있다. 데이터레이크, snapshot, backtest, validation, portfolio risk layer v1, walk-forward/report mart, market-structure shadow, frequency research v1 코드까지 구현됐다. 다음 운영 순서는 신규 SQL 적용, 1H/15m raw backfill 저장, research_1h 검증이다.
...
| latest params | code 기준 `arena-params-v7` |
| latest risk model | `portfolio-risk-v1` |
| latest feature set | code 기준 `arena-features-v4` |
...
2. 파라미터 인벤토리와 상수화
   ...
   - 현재 코드 기준은 `arena-ec2-v6`, `arena-params-v7`, `arena-features-v4`, `portfolio-risk-v1`.
...
먼저 아래 migration 두 개를 Supabase SQL Editor에서 실행한다.

- `/Users/giwon/code/news/supabase/migrations/20260620_arena_market_structure_v1.sql`
...
### 1. Frequency raw OHLCV 저장

목표:

- 4H live cycle은 건드리지 않고 1H/15m research raw만 쌓는다.
- 1H는 180일, 15m는 최소 90일 이상 확보한다.
