...
Arena는 EC2 상시 프로세스에서 `BTCUSDT` 4H paper trading을 돌고 있다. 데이터레이크, snapshot, backtest, validation, portfolio risk layer v1, walk-forward/report mart, market-structure shadow, frequency research v1, realtime execution gate v1 코드까지 구현됐다. 다음 운영 순서는 realtime execution SQL 적용, shadow gate row 확인, realtime collector enable 판단이다.
...
| latest params | code 기준 `arena-params-v9` |
...
2. 파라미터 인벤토리와 상수화
   ...
   - 현재 코드 기준은 `arena-ec2-v7`, `arena-params-v9`, `arena-features-v5`, `portfolio-risk-v1`.
