...
**P4 과최적화 감사 후속(2026-08-04) 완료.** 문서화된 누적 사양탐색 횟수 하한을
복원해 현재 기본 설정을 재평가한 결과 `fng_contrarian` DSR=0.099(N≥34), `vix_rsi`
DSR=0.134(N≥26)로 둘 다 기준 0.95에 미달했다. 두 알고를 후속 연구의 "검증된 성공
기준선"으로 쓰지 않으며, 추가 4h 파라미터 그리드는 중단한다. 상세:
[p4-overfitting-audit-20260804.md](p4-overfitting-audit-20260804.md). 다음 P2는 고정 사양의
상승장·하락장 edge/cost 생존성 감사로 제한한다.

**P2 엣지/비용 감사 후속(2026-08-04) 완료.** 3자산 포트폴리오 모두 상승장·하락장
양쪽에서 `edge/cost≥3`을 유지하지 못했다. 알고×자산의 통계적 확정 통과도 0건이다.
SOL `multi_factor`만 양쪽 포인트 기준을 넘었으나 부트스트랩 하단이 모두 0 아래라
고정 사양 관찰 후보로만 남긴다. 상세:
[p2-edge-cost-audit-20260804.md](p2-edge-cost-audit-20260804.md). P1·P4·P2가 모두 닫혀,
다음 분기는 추가 4h 튜닝이 아니라 고정 1d 비교 또는 전략 확장 종료다.

**1d 주기 비교 후속(2026-08-06) 완료.** 4h 원자재를 완전한 UTC 캘린더데이로
재표본화해 동일 알고 사양으로 재실행한 결과, 1d에서도 3자산 포트폴리오 모두 양쪽
시장창에서 `edge/cost≥3`을 만족하지 못했고 알고×자산 수준의 robust 통과도 0건이었다
(4h와 동일 결론). 상세: [p2-1d-frequency-audit-20260806.md](p2-1d-frequency-audit-20260806.md).
사전 기준대로 **현재 전략군(4h/1d 6알고 스팟 롱온리)의 확장 연구를 종료**하는 것이
타당하며, 다음 레버는 파라미터 탐색이 아니라 P5(청산 데이터 소스)·P6(숏/스테이블
슬리브) 같은 사용자 결정 항목이다.
