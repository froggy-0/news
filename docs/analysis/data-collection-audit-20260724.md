...
| # | 항목 | 심각도 | 공수 | 근거 |
|---|---|---|---|---|
| 1 | 뉴스 감성 **confidence 가중 평균** 도입 + 예측력 A/B | 🔴 높음 | 소~중 | `public_site.py:555` (§1.2) |
| 2 | 뉴스 기사 **dedup** 확인·보강 | ⚠️ 중 | 소 | `pipeline.py` (§3.3) |
| 3 | `exchange_outflow` **결정**(구현 or 제거) | ⚠️ 중 | — | `exchange_outflow.py` (§3.1) |
| 4 | median 계산 `statistics.median()`로 수정 | 🟢 낮음 | 소 | `public_site.py:573` (§1.3) |
...
**핵심 메시지**: 구조·방어·누수방지·이상치처리는 이미 잘 돼 있다. 유일하게 **구조적으로 아쉬운 지점은
뉴스 감성 집계가 confidence·소스·최신성·중복을 무시한 단순 평균**이라는 것 — 이게 PCA 지수 상류라
개선 시 파급이 크다. 나머지는 유지보수·정리 수준.
...
- **PLAUSIBLE**(코드 근거 있으나 실측/표본 검증 권장): §2.4(stale 빈도), §3.3(dedup 부재), §1.2의 "예측력 저하"
  실제 크기 — 모두 표본 데이터로 정량 확인 필요.
