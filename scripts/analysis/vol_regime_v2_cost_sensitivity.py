...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="master parquet 경로 (sentinel 파일명)",
    )
    ...
    print(f"\n=== Transaction Cost Sensitivity · {parquet_path.name} ===")
    ...
    for name in baseline_names:
        ...
        # Breakeven fee: 전략 Sharpe가 always_up 이하가 되는 수수료 찾기
        be_fee = _find_breakeven_fee(df, signal, args.return_col, always_up(df))
        if be_fee is not None:
            print(f"  → breakeven fee ≈ {be_fee:.1f} bps/leg")
        print()
