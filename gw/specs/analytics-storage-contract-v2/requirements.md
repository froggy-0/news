...
8. WHEN curated JSON의 `newsSentiment`에 `median`, `bullishRatio`, `bearishRatio` 등 전시 전용 필드가 포함되더라도, THE analytics writer SHALL `mean`, `std`, `count`만 추출한다.
9. WHEN curated JSON에 `signalSentiment`, `signalSentimentStatus` 등 X 시그널 감성 필드가 포함되더라도, THE analytics writer SHALL 이를 포함하지 않는다. 시그널 감성은 현재 통계 검정 대상이 아니며 수집 경로 안정성이 낮다.
